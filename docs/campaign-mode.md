...
4. 本次发射未命中任何目标时，玩家硬币停止或掉出桌外后扣 1 点生命。
5. 目标被命中后继续参与物理运动，不会在碰撞瞬间消失。
6. 任意层级被连锁命中的目标在桌面停止后消失，不扣生命。
7. 任意层级被连锁命中的目标掉出桌外后消失，并分别扣 1 点生命。
8. 生命归零时立即失败。扣生命的判定优先于过关判定，所以最后一枚目标掉落且正好耗尽最后一点生命时，结果为失败。
9. 生命仍有剩余且所有目标都已消失时，闯关成功。
10. 玩家硬币掉落且关卡仍可继续时，会回到本关配置的初始位置。
